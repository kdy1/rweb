...
impl FormatText {
    ...
    fn render_request(&mut self, now: time::Tm, req: &ServiceRequest) {
        match *self {
            ...
            FormatText::RequestTime => *self = FormatText::Str(now.rfc3339().to_string()),
...
pub(crate) struct FormatDisplay<'a>(&'a dyn Fn(&mut Formatter<'_>) -> Result<(), fmt::Error>);
...
mod tests {
    ...
    use rweb_service::{IntoService, Service, Transform};
