...
///
/// ```rust
/// use rweb::{guard, http, middleware, App, HttpResponse};
///
/// fn main() {
///     let app = App::new()
...
///                 .route(guard::http::method(http::Method::HEAD).to(|| HttpResponse::MethodNotAllowed()))
///         );
/// }
/// ```
#[derive(Clone)]
...
mod tests {
    ...
    async fn test_default_headers() {
        ...
        let srv = |req: ServiceRequest| {
            ok(req.into_response(HttpResponse::Ok().header(CONTENT_TYPE, "0002").finish()))
        };
        let mut mw = DefaultHeaders::new()
            .header(CONTENT_TYPE, "0001")
