...
///
/// [**PathConfig**](struct.PathConfig.html) allows to configure extraction
/// process.
///
/// ## Example
///
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        ...
        ready(
            de::Deserialize::deserialize(PathDeserializer::new(req.match_info()))
                ...
                .map_err(move |e| {
                    log::debug!(
                        "Failed during Path extractor deserialization. Request path: {:?}",
                        req.path()
                    );
...
mod tests {
    ...
    async fn test_tuple_extract() {
        ...
        let res = <(Path<(String, String)>, Path<(String, String)>)>::from_request(&req, &mut pl)
            .await
            .unwrap();
        assert_eq!((res.0).0, "name");
        assert_eq!((res.0).1, "user1");
