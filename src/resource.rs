...
use rweb_service::{
    ...
    IntoServiceFactory, Service, ServiceFactory, Transform,
};
use std::{
    cell::RefCell,
    fmt,
...
type HttpService = BoxService<Req, Resp, Error>;

/// *Resource* is an entry in resources table which corresponds to requested
/// URL.
///
    ...
    T: ServiceFactory<Config = (), Request = Req, Response = Resp, Error = Error, InitError = ()>,
{
    ...
    ///
    /// This is similar to `App's` middlewares, but middleware get invoked on
    /// resource level. Resource level middlewares are not allowed to change
    /// response type (i.e modify response's body).
    ///
    /// **Note**: middlewares get called in opposite order of middlewares
    /// registration.
    pub fn wrap<M>(
        self,
        mw: M,
    ) -> Resource<
        impl ServiceFactory<Config = (), Request = Req, Response = Resp, Error = Error, InitError = ()>,
    >
    where
        M: Transform<T::Service, Request = Req, Response = Resp, Error = Error, InitError = ()>,
    {
        Resource {
    ...
    ///
    /// This function accepts instance of `Req` type and
    ...
    >
    where
        F: FnMut(Req, &mut T::Service) -> R + Clone,
        R: Future<Output = Result<Resp, Error>>,
    {
        Resource {
...
impl Future for CreateResourceService {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        for item in &mut self.fut {
            match item {
                CreateRouteServiceItem::Future(ref mut fut) => match Pin::new(fut).poll(cx)? {
...
impl Service for ResourceService {
    ...
    type Response = Resp;
    type Error = Error;
    type Future = Either<Ready<Result<Resp, Error>>, LocalBoxFuture<'static, Result<Resp, Error>>>;
    ...
    async fn test_default_resource() {
        let mut srv = init_service(
            App::new()
                ...
                .default_service(|r: Req| ok(r.into_response(HttpResponse::BadRequest()))),
        )
        .await;
        let req = TestRequest::with_uri("/test").to_request();
        let resp = call_service(&mut srv, req).await;
