...
async fn main() {
    ...
    let routes = api.with(rweb::log("todos"));
    // Start up the server...
...
mod filters {
    ...
    fn with_db(db: Db) -> impl Filter<Extract = (Db,), Error = std::convert::Infallible> + Clone {
        rweb::any().map(move || db.clone())
    }

    fn json_body() -> impl Filter<Extract = (Todo,), Error = rweb::Rejection> + Clone {
        // When accepting a body, we want a JSON body
        // (and to reject huge payloads)...
        rweb::body::content_length_limit(1024 * 16).and(rweb::body::json())
    }
}
...
mod handlers {
    ...
    pub async fn list_todos(opts: ListOptions, db: Db) -> Result<impl rweb::Reply, Infallible> {
        ...
        let todos: Vec<Todo> = todos
            ...
            .collect();
        Ok(rweb::reply::json(&todos))
    }

    pub async fn create_todo(create: Todo, db: Db) -> Result<impl rweb::Reply, Infallible> {
    ...
    pub async fn update_todo(
        ...
        update: Todo,
        db: Db,
    ) -> Result<impl rweb::Reply, Infallible> {
    ...
    pub async fn delete_todo(id: u64, db: Db) -> Result<impl rweb::Reply, Infallible> {
